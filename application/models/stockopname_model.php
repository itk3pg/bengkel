...
class Stockopname_model extends CI_Model{
	...
	function SimpanImportData($data, $dataPost){
		$query = "insert into stock_opname (bukti, toko_kode, barang_kode, rak, shlv, urut, tanggal, stok_opname, user_id, waktu_insert, waktu_update) value('".$dataPost['tanggal_op']."', '".$data[0]."', '".$data[1]."', '-', '-', '-', '".$dataPost['tanggal_op']."', ".$data[2].", '".$this->session->userdata('username')."', NOW(), NOW()) on duplicate key update stok_opname=stok_opname+".$data[2]."";

		$this->db->query($query);
	}

	function UpdateStockSystem($data){
		$query = "update stock_opname a left join barang b on a.barang_kode=b.kode left join (select sbt.toko_kode, sbt.barang_kode, sbt.bulan, sbt.tahun, sbt.saldo_akhir_kwt from saldo_barang_toko sbt where sbt.bulan=".$data['bulan']." and sbt.tahun=".$data['tahun'].") c on a.barang_kode=c.barang_kode and a.toko_kode=c.toko_kode set a.hpp=b.hpp, a.stok_sistem=c.saldo_akhir_kwt, a.selisih=a.stok_opname-c.saldo_akhir_kwt where a.toko_kode='".$data['toko_kode']."' and a.bukti='".$data['tanggal']."'";
